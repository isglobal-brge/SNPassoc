...
\topmargin 0cm \textheight 23cm \oddsidemargin 0.5cm \textwidth
14cm

\begin{document}
\title{SNPassoc: an {\tt R} package to perform whole genome association
studies}

\author{Juan R Gonz\'alez, Llu\'is Armengol, Xavier Sol\'e, \\ Elisabet Guin\'o, Josep M Mercader, Xavier Estivill, V\'ictor Moreno}
...
The {\tt SNPassoc} package contains facilities for data
...
outcome and thousands of SNPs (Section \ref{section-Genome-based
association studies}). In a second and possibly third step,
...
first step, are genotyped (Section \ref{section-Medium/Large
association studies}). {\tt SNPassoc} is specially designed for
...
be analyzed using {\tt SNPassoc}. Different genetic models are
also implemented in the package. Analysis of multiple SNPs can be
analyzed using either haplotype or gene-gene interaction
approaches (Section \ref{section-Analysis of multiple SNPs}).
Statistical methods used in the functions are described in Section
\ref{section-Statistical Methods}. Lastly, some computational
aspects are described in Section \ref{section-Computational cost}.


\section{Data manipulation and descriptive
analysis}\label{section-Data manipulation and descriptive
analysis}

\subsection{The class {\tt snp}}
...
\noindent This work has been supported by CEGEN (Spanish
Genotyping Center) funded by Genoma Espa\~na and grant FIS03/0144
(Instituto de Salud Carlos III). JMM was supported by the CRG
under project SAF2005-01005 (Spanish Ministry of Science and
Education) and the Danone Institute.
